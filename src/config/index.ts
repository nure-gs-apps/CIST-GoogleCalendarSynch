...
export function tryGetConfigDirFromEnv() {
  return process.env[transformAppEnvConfigKey(
    iterate(objectKeys(process.env)).filter(key => isAppEnvConfigKey(key)).take(1).toArray()[0]
  )];
}
...
function initializeNconfSync<T extends IFullAppConfig>(argv: Argv<T>) {
  ...
  const files = [
    ...
    b => ['.json', '.yaml', '.yml', '.toml'].map(ext => `${b}${ext}`)
